...
#include "Ssw.h"
#include "private/ssw/ssw_impl.h"
#include <sstream>
...
int CalculateNumberMismatch(
    StripedSmithWaterman::Alignment *al, int8_t const *ref, int8_t const *query, const int& query_len
                           ) {
    ...
    for (unsigned int i = 0; i < al->cigar.size(); ++i) {
        ...
        if (op == 'M') {
            for (uint32_t j = 0; j < length; ++j) {
                if (*ref != *query
                    // && *ref != 4 /* do NOT count N in reference as mismatch */
                    ) { 
                    ++mismatch_length;
...
Aligner::Aligner(Aligner&& other) {
    if (this != &other) {
        std::swap(score_matrix_, other.score_matrix_);
        score_matrix_size_ = other.score_matrix_size_;
        std::swap(translation_matrix_, other.translation_matrix_);
        match_score_ = other.match_score_;
        gap_opening_penalty_ = other.gap_opening_penalty_;
        gap_extending_penalty_ = other.gap_extending_penalty_;
        std::swap(translated_reference_, other.translated_reference_);
        reference_length_ = other.reference_length_;
    }
}

Aligner& Aligner::operator=(Aligner&& other) {
    // assert(this != &other);
    std::swap(score_matrix_, other.score_matrix_);
    score_matrix_size_ = other.score_matrix_size_;
    std::swap(translation_matrix_, other.translation_matrix_);
...
bool Aligner::Align(const char *query, const Filter& filter, Alignment *alignment) const {
    ...
    uint8_t flag = 0;
    SetFlag(filter, &flag);
    s_align *s_al = ssw_align(profile
...
bool Aligner::Align(const char *query
                    , const char *ref
                    , const int& ref_len
                    , const Filter& filter
                    , Alignment *alignment
...
uint8_t Aligner::GetMatchScore() const {
    return match_score_;
}

uint8_t Aligner::GetMismatchPenalty() const {
    return mismatch_penalty_;
}

uint8_t Aligner::GetGapOpenPanalty() const {
    return gap_opening_penalty_;
}

uint8_t Aligner::GetGapExtPenalty() const {
