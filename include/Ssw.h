...
public:
    ...
    Aligner(Aligner&&); // move constructor

    ~Aligner(void);

    Aligner& operator=(Aligner&&); // move assignment
    ...
    uint8_t GetMatchScore() const;

    uint8_t GetMismatchPenalty() const;

    uint8_t GetGapOpenPanalty() const;

    uint8_t GetGapExtPenalty() const;
